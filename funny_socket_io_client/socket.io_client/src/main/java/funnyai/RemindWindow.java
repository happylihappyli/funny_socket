...
public class RemindWindow {
...

    {
        ...
        feaMap = new HashMap<>();
        feaMap.put("name", "FunnyMsg");
        feaMap.put("release", sdf.format(new Date()));
    }

    public void Show(String strTitle,String strMsg) {
        feaMap.put(
                "feature",
                strMsg);
        init(strTitle);
    ...
    public void init(String strMsg) {
        // 新建300x220的消息提示框
...
//        imgLabel = new JLabel(img);
        ...
        ((JPanel) tw.getContentPane()).setOpaque(true);//false);
...
//        new Test();
        RemindWindow reminUtil = new RemindWindow();
        reminUtil.Show("test","test");
    }
}
